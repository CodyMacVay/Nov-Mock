...
function reg_user($conn){
    ...
    $sql = "INSERT INTO user (email, password, fname, sname, dob, sign_up, address, postcode, county, phone) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";  //prepare the sql to be sent
...
function staf_geter($conn){
    ...
    $sql = "SELECT staffid, role, fname, sname FROM staff WHERE role != ? ORDER BY role DESC";
...
function appt_getter($conn){
    ...
    $sql = "SELECT b.bookid, b.appointmentdate, b.bookedon, s.role, s.fname, s.sname FROM book b JOIN staff s ON b.staffid = s.staffid WHERE b.userid = ? 
